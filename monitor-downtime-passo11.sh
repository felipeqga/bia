...
# Script de Monitoramento de Downtime - PASSO-11 Dockerfile HTTPS
# Alteração: VITE_API_URL para https://desafio3.eletroboards.com.br

ALB_DNS="bia-1751550233.us-east-1.elb.amazonaws.com"
API_ENDPOINT="http://${ALB_DNS}/api/versao"
LOG_FILE="/home/ec2-user/bia/downtime-monitor-passo11-$(date +%Y%m%d-%H%M%S).log"

echo "🚀 MONITORAMENTO DE DOWNTIME - PASSO-11 DOCKERFILE HTTPS" | tee -a $LOG_FILE
echo "⏰ Iniciado em: $(date)" | tee -a $LOG_FILE
echo "🎯 Endpoint: $API_ENDPOINT" | tee -a $LOG_FILE
echo "📊 Alteração: Dockerfile VITE_API_URL → https://desafio3.eletroboards.com.br" | tee -a $LOG_FILE
